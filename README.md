...
## Current status: Milestone 21 complete ✅ (22 in progress)
...
- [ ] Dynamic linking is **in progress**: `ld-linux.so.2` loads, runs and
      maps `libc.so.6` correctly, but symbol resolution does not complete,
      so a dynamically linked program does not run yet (ROADMAP 22)
