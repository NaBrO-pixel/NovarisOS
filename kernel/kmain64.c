...
extern const unsigned char scm64_elf[];
...
extern const unsigned char prctl64_elf[];
extern const unsigned long prctl64_elf_len;
extern const unsigned char tgkill64_elf[];
extern const unsigned long tgkill64_elf_len;
...
void kernel_main(uint32_t magic, void* mbi) {
    ...
    /* --- layer 33: starting another program (Milestone 73) ----------- */
    /* posix_spawn, which glibc issues as
     * clone(CLONE_VM|CLONE_VFORK|SIGCHLD) - neither of the two shapes
     * this kernel's clone knew. It has CLONE_VM, so it was answered as
     * a thread, with a tid; the caller then asked wait4 for that tid and
     * got -ECHILD, because a thread is not a child.
     *
     * That is exactly where Milestone 72 left wineboot: it built its
     * prefix and then could not start the wineserver. Nothing on the
     * Windows side runs until this does.
     *
     * The program below execs /tmp/forkchild64, which the initrd already
     * stages for the fork/exec layer. */
    serial64_puts("NOVARIS64: -- starting another program --\n");
    {
        const uint64_t STACK_TOP   = 0x00007FFFFFFF0000ULL;
        const uint64_t STACK_PAGES = 64;
        vmspace64_t kspace;
        elf64_info_t info;
        uint64_t i, rsp;
        uint64_t vforks_before;
        int rc, stack_ok = 1, pid;
        proc64_t* p;
        static const char* const spawn_argv[] = { "/spawn64", 0 };
        ...
        check("a space for it", vmspace64_create(&p->space) != 0);
        rc = elf64_load(spawn64_elf, spawn64_elf_len, &p->space, &info);
        check("the spawn program loaded", rc == ELF64_OK);
        ...
        check("the spawns behaved as Linux's do",
              syscall64_exit_code() == 103);

        /* And they were really vforks. Without this the layer passes on
         * a kernel that quietly routed CLONE_VFORK to the ordinary fork
         * path - which would also produce a waitable child, and would
         * also let the parent run while the child was still standing on
         * a stack it was about to free. Two spawns, two vforks. */
        serial64_puts("NOVARIS64: vforks  = ");
        serial64_putdec(syscall64_vforks() - vforks_before);
        serial64_putc('\n');
        check("each spawn suspended its parent",
    ...
    /* --- layer 36: passing a descriptor (Milestone 76) --------------- */
    /* Where Milestone 75 stopped, and the mechanism the whole Windows
     * side stands on: every Windows object a Wine process holds is,
     * underneath, a file descriptor the wineserver sent it over their
     * socket with SCM_RIGHTS.
     *
     * It is the first operation in this tree that touches two
     * processes' descriptor tables. What travels is the descriptor and
     * not the number - the sender names one of its own, the receiver
     * gets the thing it referred to at a number the receiver picks - so
     * the assertions are about identity rather than arrival. A test that
     * only checked "a descriptor appeared" would pass on an
     * implementation that sent the integer. */
    serial64_puts("NOVARIS64: -- passing a descriptor --\n");
    {
        const uint64_t STACK_TOP   = 0x00007FFFFFFF0000ULL;
        const uint64_t STACK_PAGES = 64;
        vmspace64_t kspace;
        elf64_info_t info;
        uint64_t i, rsp;
        uint64_t passed_before, sent_before;
        int rc, stack_ok = 1, pid;
        proc64_t* p;
        static const char* const scm_argv[] = { "/scm64", 0 };
        ...
        sock64_init();
        vmspace64_kernel_space(&kspace);
        passed_before = syscall64_fds_passed();
        sent_before   = syscall64_sendmsgs();
        ...
        check("a space for it", vmspace64_create(&p->space) != 0);
        rc = elf64_load(scm64_elf, scm64_elf_len, &p->space, &info);
        check("the descriptor-passing program loaded", rc == ELF64_OK);
        ...
        /* The timer has to be running for this one, and that is not a
         * detail. Every layer before this waited for an *event* - a
         * write, an exit, a connection - and something else always
         * caused it. This program waits for *time*: it sleeps so that
         * its child reaches recvmsg and blocks there before anything is
         * sent, which is the only way to exercise a receive that has to
         * wait. A deadline cannot be reached with IRQ0 masked, because
         * nothing advances the counter it is measured against - the
         * sleep would not be slow, it would never end.
         *
         * Ticks land only in ring 3: the syscall path clears IF on the
         * way in, so a task is never preempted inside the kernel, which
         * is what makes one kernel stack safe. Same arrangement as the
         * preemption layer, and masked again afterwards so no tick
         * arrives during the assertions below. */
        register_interrupt_handler64(32, sched_timer_handler);
        idt64_irq_set_mask(0, 0);
        ...
        check("descriptors passed as Linux's do",
              syscall64_exit_code() == 113);

        /* Two descriptors, over three sendmsgs - the first carries only
         * bytes. Counted because a sendmsg that delivered the payload
         * and quietly dropped the control data would satisfy every
         * assertion the program makes about the *bytes*, and the
         * program's own checks would then fail somewhere further along
         * looking like a broken file rather than a lost descriptor. */
        serial64_puts("NOVARIS64: passed  = ");
        serial64_putdec(syscall64_fds_passed() - passed_before);
        serial64_puts(" descriptors over ");
        serial64_putdec(syscall64_sendmsgs() - sent_before);
        serial64_puts(" sendmsgs\n");
        check("two descriptors were carried, over three messages",
              syscall64_fds_passed() - passed_before == 2 &&
              syscall64_sendmsgs() - sent_before == 3);

        serial64_puts("NOVARIS64: live    = ");
        serial64_putdec(pipe64_live());
        serial64_puts(" pipes still open\n");
        check("and nothing leaked", pipe64_live() == 0);
    }

    /* --- layer 37: shared memory (Milestone 78) ---------------------- */
    /* Milestone 78 made five fixes and shipped none of these assertions,
     * which is the wrong way round and is what this layer repays.
     *
     * Two mechanisms are worth asserting because both were wrong. A file
     * unlinked while open has to go on existing until the last
     * descriptor closes - ramfs64_unlink used to free it immediately and
     * its own comment predicted the consequence. And a MAP_SHARED
     * mapping has to be shared, which is the harder one to test: a
     * private copy satisfies every read and write a single process can
     * make, so **only a second process can tell the difference**, and
     * the assertions that matter all run across a fork.
     *
     * They are one idiom rather than two features. Create, size, unlink,
     * map is how a program gets anonymous shared memory out of a
     * filesystem, and it is what Wine does for every shared mapping it
     * makes; neither half is any use without the other. */
    serial64_puts("NOVARIS64: -- shared memory --\n");
    {
        const uint64_t STACK_TOP   = 0x00007FFFFFFF0000ULL;
        const uint64_t STACK_PAGES = 64;
        vmspace64_t kspace;
        elf64_info_t info;
        uint64_t i, rsp;
        uint64_t shared_before;
        int rc, stack_ok = 1, pid;
        proc64_t* p;
        static const char* const shm_argv[] = { "/shm64", 0 };
        ...
        sock64_init();
        vmspace64_kernel_space(&kspace);
        shared_before = syscall64_shared_maps();
        ...
        check("a space for it", vmspace64_create(&p->space) != 0);
        rc = elf64_load(shm64_elf, shm64_elf_len, &p->space, &info);
        check("the shared-memory program loaded", rc == ELF64_OK);
        ...
        check("shared memory behaved as Linux's does",
              syscall64_exit_code() == 119);

        /* Three: the parent's, the child's, and the fixed one. Counted
         * because a MAP_SHARED quietly answered with a private copy
         * would satisfy every assertion a single process makes, and the
         * program only catches it by having a second process look. */
        serial64_puts("NOVARIS64: shared  = ");
        serial64_putdec(syscall64_shared_maps() - shared_before);
    ...
    /* --- kill(2), one process signalling another (Milestone 93) ------ *
     *
     * Everything this kernel could do with a signal before now was
     * synchronous: a fault, delivered to the thread that caused it. Its
     * only signal calls were tkill and tgkill, and both begin by
     * refusing any tid but the caller's - so nothing could signal
     * anything but itself, and chrome.exe asking for kill(2) got
     * -ENOSYS twice in a run.
     *
     * This layer forks, signals across the gap, and checks the answers
     * against Linux's. It needs the timer for the same reason the clock
     * layer does: the child waits in nanosleep, and a sleep ends when
     * the tick counter says so. */
    serial64_puts("NOVARIS64: -- kill --\n");
    {
        const uint64_t STACK_TOP   = 0x00007FFFFFFF0000ULL;
        const uint64_t STACK_PAGES = 64;
        vmspace64_t kspace;
        ...
        int rc, stack_ok = 1, pid;
        proc64_t* p;
        static const char* const kill_argv[] = { "/kill64", 0 };
        ...
        check("a space for it", vmspace64_create(&p->space) != 0);
        rc = elf64_load(kill64_elf, kill64_elf_len, &p->space, &info);
        check("the kill program loaded", rc == ELF64_OK);
        ...
        check("kill(2) behaved as Linux's does",
              syscall64_exit_code() == 127);
    ...
    /* --- prctl's PR_SET_NAME (Milestone 95) -------------------------- *
     *
     * chrome.exe asked for prctl three times and got -ENOSYS each time.
     * Two of Wine's three call sites are PR_SET_NAME, and the preloader
     * is the one that checks the answer. */
    serial64_puts("NOVARIS64: -- prctl --\n");
    {
        const uint64_t STACK_TOP   = 0x00007FFFFFFF0000ULL;
        const uint64_t STACK_PAGES = 64;
        vmspace64_t kspace;
        ...
        int rc, stack_ok = 1, pid;
        proc64_t* p;
        static const char* const prctl_argv[] = { "/prctl64", 0 };
        ...
        check("a space for it", vmspace64_create(&p->space) != 0);
        rc = elf64_load(prctl64_elf, prctl64_elf_len, &p->space, &info);
        check("the prctl program loaded", rc == ELF64_OK);
        ...
        check("prctl behaved as Linux's does",
              syscall64_exit_code() == 127);
    }

    /* --- tgkill's thread target (Milestone 96) ----------------------- *
     *
     * tkill and tgkill both began `if (tid != proc64_current_pid())
     * return -ESRCH`, which compares a task slot against a process id
     * and refuses every thread but the caller. Nothing here could
     * signal a sibling thread at all.
     *
     * That is where chrome.exe stopped. Chromium's sampling profiler
     * suspends the main thread; Wine does that by sending the target
     * SIGUSR1; -ESRCH left the wineserver believing the thread had
     * died while the thread ran on marked suspended - and a suspended
     * thread is never allowed to acquire anything, so every wait it
     * made afterwards was answered PENDING. The visible end of it was
     * chrome.exe waiting forever on a mutex it had just created
     * unowned.
     *
     * Needs the timer: the second thread sleeps in read(2) and the
     * point of the layer is that a signal reaches it there. */
    serial64_puts("NOVARIS64: -- tgkill --\n");
    {
        const uint64_t STACK_TOP   = 0x00007FFFFFFF0000ULL;
        const uint64_t STACK_PAGES = 64;
        vmspace64_t kspace;
        ...
        int rc, stack_ok = 1, pid;
        proc64_t* p;
        static const char* const tgkill_argv[] = { "/tgkill64", 0 };
        ...
        check("a space for it", vmspace64_create(&p->space) != 0);
        rc = elf64_load(tgkill64_elf, tgkill64_elf_len, &p->space, &info);
        check("the tgkill program loaded", rc == ELF64_OK);
        ...
        check("tgkill behaved as Linux's does",
              syscall64_exit_code() == 127);
    }
    ...
    serial64_puts("NOVARIS64: -- a Wine prefix --\n");
    {
        ...
        if (ramfs64_lookup("/usr/bin/x86_64-windows/wineboot.exe") < 0) {
            ...
            if (rc == ELF64_OK && stack_ok && rsp) {
                 ...
                 * With IRQ0 masked it is not a thirty-second wait, it is
                 * a permanent one. */
                register_interrupt_handler64(32, sched_timer_handler);
                idt64_irq_set_mask(0, 0);
