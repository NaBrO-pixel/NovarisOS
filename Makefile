...
OBJS = $(BUILD_DIR)/boot.o $(BUILD_DIR)/kernel.o \
       ...
       $(BUILD_DIR)/syscall.o $(BUILD_DIR)/posix.o $(BUILD_DIR)/process.o $(BUILD_DIR)/process_asm.o \
...
$(BUILD_DIR)/initrd_staging/helloelf.elf: $(BUILD_DIR)/user/hello_c.bin \
        $(BUILD_DIR)/user/hello_elf.elf $(BUILD_DIR)/user/posixtest.elf \
...
.PHONY: test-posix
test-posix: $(ISO) $(BUILD_DIR)/user/posixtest.elf
	python3 tools/posix_compare.py --iso $(ISO) \
	    --binary $(BUILD_DIR)/user/posixtest.elf
